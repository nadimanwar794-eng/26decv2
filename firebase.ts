...
export const bulkSaveLinks = async (updates: Record<string, any>) => {
  try {
    // RTDB: FLATTEN UPDATES TO PRESERVE EXISTING FIELDS (MERGE LOGIC)
    // Instead of replacing the whole node, we update specific sub-paths.
    const rtdbUpdates: Record<string, any> = {};
    Object.entries(updates).forEach(([key, data]) => {
         // data is { freeLink: "...", premiumLink: "...", price: ... }
         Object.entries(data).forEach(([field, value]) => {
             // Update specific fields under content_data/KEY
             rtdbUpdates[`content_data/${key}/${field}`] = value;
         });
    });
...
export const saveChapterData = async (key: string, data: any) => {
  try {
    // RTDB: Use UPDATE instead of SET to allow partial updates if needed,
    // though Admin usually sends full object. This is safer against accidental wipes.
