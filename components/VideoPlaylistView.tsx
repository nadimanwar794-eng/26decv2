...
export const VideoPlaylistView: React.FC<Props> = ({ 
  ...
  const confirmPayment = () => {
      ...
      setConfirmModal(null);
      setActiveVideo(playlist[confirmModal.index]);
  };
  ...
  return (
    <div className="bg-white min-h-screen pb-20 animate-in fade-in slide-in-from-right-8">
       ...
       {/* PLAYER AREA */}
       <div className="aspect-video bg-black w-full sticky top-[73px] z-10">
           {activeVideo ? (
               <iframe 
           ...
           {loading ? (
               ...
               <div className="grid grid-cols-1 gap-3">
                   {Array.from({length: 10}).map((_, idx) => {
                       const vid = playlist[idx] || { url: '', price: settings?.defaultVideoCost ?? 5 };
                       const hasLink = !!vid.url;
                       ...
                       return (
                           <button 
                               key={idx}
                               onClick={() => hasLink ? handleVideoClick(idx) : alert("This video will be available soon.")}
                               disabled={!hasLink && user.role !== 'ADMIN'} // Admin can click to verify even if empty logic needed? No, standard user logic.
                           ...
                           >
                               <div className={`w-10 h-10 rounded-full flex items-center justify-center ${!hasLink ? 'bg-slate-200 text-slate-400' : isActive ? 'bg-red-600 text-white' : 'bg-slate-100 text-slate-400'}`}>
                                   {isActive ? <PlayCircle size={20} fill="currentColor" /> : <span className="font-bold text-sm">{idx + 1}</span>}
                               </div>
                               <div className="flex-1">
                                   <h5 className={`font-bold text-sm ${!hasLink ? 'text-slate-400' : isActive ? 'text-red-700' : 'text-slate-700'}`}>
                                       Video Lecture {idx + 1}
                                   </h5>
                                   <div className="flex items-center gap-2 mt-1">
                                       {!hasLink ? (
                                            <span className="text-[10px] font-bold bg-slate-100 text-slate-400 px-2 py-0.5 rounded flex items-center gap-1">
                                               Coming Soon
                                           </span>
                                       ) : isFree ? (
