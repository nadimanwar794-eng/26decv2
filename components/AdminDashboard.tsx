...
<html lang="en">
  ...
  const loadChapterContent = async (chId: string) => {
      ...
      try {
          ...
          setIsContentLoading(false); // Unlock inputs ALWAYS
      }
  };
                                          ...
                                          <div key={i} className="flex gap-2 items-center bg-white p-2 rounded-lg border border-rose-100">
                                              ...
                                              <input 
                                                  ...
                                                  onChange={(e) => {
                                                      ...
                                                      // Ensure the array is padded up to the index we are editing
                                                      // This prevents gaps and ensures persistence logic works for any slot
                                                      while(updated.length <= i) updated.push({title:`Video ${updated.length+1}`, url:'', price:5});
                                                      
                                                      // Update specific slot
                                                      // We maintain 'title' even if hidden, just in case
                                                      const current = updated[i] || {title: `Video ${i+1}`, url: '', price: 5};
                                                      updated[i] = {...current, url: e.target.value, title: `Video ${i+1}`}; // Enforce title consistency
