...
export const PdfView: React.FC<Props> = ({ 
  ...
  const handlePdfClick = (type: 'FREE' | 'PREMIUM' | 'ULTRA') => {
      ...
      if (type === 'FREE') {
          ...
          price = contentData?.ultraPdfPrice || 10;
      }
       ...
       <div className="p-6 space-y-4">
           {loading ? (
               ...
               <>
                   ...
                   {/* ULTRA PDF */}
                   <button 
                       onClick={() => handlePdfClick('ULTRA')}
                       className="w-full p-5 rounded-2xl border-2 border-indigo-100 bg-indigo-50/30 hover:bg-indigo-50 flex items-center gap-4 transition-all relative overflow-hidden"
                   >
                       <div className="absolute top-0 right-0 bg-indigo-600 text-white text-[9px] font-bold px-3 py-1 rounded-bl-xl">EXCLUSIVE</div>
                       ...
                       <div className="flex-1 text-left">
                           ...
                           <p className="text-xs text-slate-500">Top Ranker's Material</p>
                       </div>
                       <span className="text-[10px] font-bold bg-indigo-200 text-indigo-800 px-3 py-1 rounded-full flex items-center gap-1">
                           <Lock size={10} /> {contentData?.ultraPdfPrice || 10} CR
                       </span>
                   </button>
               </>
           )}
